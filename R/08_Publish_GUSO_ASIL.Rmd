...
Here we show a more refined approach for obtaining summarizing data of time series, based on smoothing over time using the R-packages `mgcv` and the function `gam()`.
These data are extracted using spline smoothing and only the offsets of the smooths are retained, representing the time series means (TSM).
figure 4 shows the smooths for the performance of participant 9, in condition 1 (2D), trials 1 to 4, belonging to group 11 (first violin, first piece).

![Hypothesis testing](FiguresPaper/smooths_participantconditiontrial9.png)
The offsets (called: TSMs) of these smooths are:

```{r offsets, echo=FALSE}
load(file = "Fit/GAM_procustus_G_11")
G_11$coefficients["participantconditiontrial9.1.1"]
G_11$coefficients["participantconditiontrial9.1.2"]
G_11$coefficients["participantconditiontrial9.1.3"]
G_11$coefficients["participantconditiontrial9.1.4"]
```

When apply this approach to all participants, conditions and trials, we obtain values that can be compared with the values obtaind by just taking the median values over time. Accordingly, the figure (labelled Figure 4) shows the correlation of TSM with 0M and 1M. 

![Hypothesis testing](FiguresPaper/CopyOfresponse_correlation_TSM_M_procustus.png)
...
```{r Bayes-factor1M, echo=FALSE, message=TRUE}
...
### Bayes-factor TSM models

```{r Bayes-factorTSM, echo=FALSE, message=TRUE}
load(file = "Results/BF_model_comparison_procustusTSM.RData")
print(paste("Bayes factor in favor of model_1 over model_2 (procustusTMS): ", BF_model_comparison_procustusTSM$bf ))
```
...
```{r DiagnostiicsTSM, echo=FALSE}
load(file = "Results/post_analysis_procustusTSM.RData")
...
![Posterior distributions of condition and trials -- procustusTSM](FiguresPaper/ConditionTrialsPostDistr_procustusTSM.png)
...
Here we show the contrasts of the TSM modelling.
Resuts are in line with the calibrated procustus.


```{r ContrastsTSM, echo=TRUE}
load(file = "Results/hypothesis_test_procustusTSM.RData")

hypothesis_test_procustusTSM[[1]][,1]
hypothesis_test_procustusTSM[[1]][,-1]
hypothesis_test_procustusTSM[[2]]
...
```{r ContrastsQuestionnaires, echo=FALSE}
