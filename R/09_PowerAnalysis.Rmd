...
# hier nog de creatie van de dataset
    ...
    condition2 <- rnorm(total/2,0,1) %>% data.frame(condition.value = .) #%>% mutate(condition.name = "condition2")
    condition <- rbind(condition1,condition2) 
   ggplot(sim_dataset) +
  geom_histogram(aes(response, color = condition.name),bins = 30)
 

sim_form <- bf(response ~ 0 + condition.name + (1 | participant.name + trial.name))
sim_prior <- c(brms::prior(normal(0.5, 1), class = "b"))

ggplot(sim_dataset) +
  geom_histogram(aes(response, color = condition.name),bins = 30)


sim_model <- run_model_cmdstanr(sim_dataset,sim_form,"gaussian",sim_prior)
```
...
m_diff = 0.3
    ...
    condition2 <- rnorm(total/2,0,1) %>% data.frame(condition.value = .) #%>% mutate(condition.name = "condition2")
    condition <- rbind(condition1,condition2) 
    cohend = m_diff /sd(condition$condition.value)
    cohend
...
resp <- Data$response1M_procustus

cor(wpq,mpqs, use = "complete")
cor(wpq,mpqp, use = "complete")
cor(mpqs,mpqp, use = "complete")

cor(wpq,resp, use = "complete")
cor(mpqs,resp, use = "complete")
cor(mpqp,resp, use = "complete")

ggplot() +
  geom_point(aes(x=wpq,y=mpqs))

ggplot() +
  geom_point(aes(x=wpq,y=mpqp))

ggplot() +
  geom_point(aes(x=mpqs,y=mpqp))
