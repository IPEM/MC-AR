...
#3.	The R Package:
#3.1.	Description:
The R package tests four different models against four distinct hypotheses. In the initial two models, kinematic metrics describing the student's performance relative to the virtual teacher, as shown in (1. The Unity package), serve as response variables in the models. The predictors include conditions, participants, and trial information. These two models are referred to as Model_1 and Model_2, with the primary difference being the inclusion of perceived difficulty in the interaction with the condition in Model_2.
...
Hypothesis 4: This hypothesis investigates the extent to which the degree of presence in augmented reality (AR) influences students' violin performance. This evaluation involves comparing Model_3 and Model_4." R package tests 4 different models in 4 different hypotheses. In the first 2 models, the kinematic metrics describing the performance of the student relative to the virtual teacher shown in (1. The Unity package) are response variables in the models, while predictors include conditions, participants, and trial information. There are two models: Model_1 and Model_2, with the key difference being the inclusion of perceived difficulty in the interaction with the condition in Model_2.
#3.2.	Impact overview:
In this code and the associated work, we employ Bayesian statistics. These methodologies have not yet been fully integrated into current research practices. Bayesian statistics can be especially valuable for small sample sizes and prove particularly advantageous in fields where collecting large datasets is challenging or expensive, such as multidisciplinary research projects involving non-user-friendly prototypes of new technology [16].
#3.3.	Current use:
